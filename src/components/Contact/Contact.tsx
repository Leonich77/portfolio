...
export const Contact = () => {
  const { contactTitle, contactDescription, socialLinks } = config;
  const emailRef = useRef<null | HTMLInputElement>(null);
  const messageRef = useRef<null | HTMLTextAreaElement>(null);

  const getSocialType = (link: string) => {
    const domain = link.split(".")[0].replace("https://", "");
  ...
  return (
    <section className="contact">
      <div className="container">
        <Section title={contactTitle} description={contactDescription} />
